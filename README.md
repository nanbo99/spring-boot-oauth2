...
```sh
curl -vu rajithapp:secret http://localhost:9191/api/oauth/token --data "username=admin&password=admin&grant_type=password"
...
```sh
curl -vu rajithapp:secret http://localhost:9191/api/oauth/token  --data "grant_type=refresh_token&refresh_token=<refresh_token>"
